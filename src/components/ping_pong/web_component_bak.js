return function() {
  ...
  const Stream = new EventBus();

  //---------------------------------------------------------
  //---------- Mimics Arcadia way of querying data ----------
  //---------------------------------------------------------
  /* Link: http://documentation.arcadiadata.com/latest/#pages/topics/arcapi-cust-exten-js-apis.html#arcapi-cust-exten-js-apis-dataresult-functions */
  ...
  //---------- Queries data (csv) ----------
  //----------------------------------------
  /* This is only used for local testing with csv */
  async function getFileData(url, format = "csv") {
    return new Promise((resolve, reject) => {
      d3[format](url, (err, data) => {
        if (err) {
          reject(err);
        } else {
          resolve(data);
  ...
  const service = {
    getHourlyBaseline: async () => {
      const data = await getData({
        ...
        dimensions: [
          {
            type: "SIMPLE",
            expr: "[Datehour] as 'Datehour'"
          },
          {
            type: "SIMPLE",
            expr: "[Currenthour] as 'Currenthour'"
          },
          {
            type: "SIMPLE",
            expr: "[sess_len] as 'sess_len'"
          },
          {
            type: "SIMPLE",
            expr: "[avg_fea] as 'avg_fea'"
          },
          {
            type: "SIMPLE",
            expr: "[avg_cust] as 'avg_cust'"
          }
        ],
        dataset_id: 105
      });
    ...
    getDaysSimulated: async () => {
      const data = await getData({
        ...
        dimensions: [
          {
            type: "SIMPLE",
            expr: "[ROWTIME] as 'ROWTIME'"
          },
          {
            type: "SIMPLE",
            expr: "[ROWKEY] as 'ROWKEY'"
          },
          {
            type: "SIMPLE",
            expr: "[VALUE] as 'VALUE'"
    ...
    getActCodes: async () => {
      return getConfigData("act_codes");
    },
    getStyles: async () => {
      return getConfigData("styles");
    },
    getValcols: async () => {
      return getConfigData("valcols");
    }
  };
  ...
  // afterDraw: always executes after the draw base visual function called.
  f.afterDraw = async function() {};
